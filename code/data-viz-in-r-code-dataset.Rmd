...
Get the IPEDS Admissions data for institutions of interest for survey years 2017-2020 and process to create a tidy dataset.
...
adm_20 <- read_csv(here('data', 'ipeds_2020', 'adm2020.csv')) %>% 
  janitor::clean_names() %>% mutate(year = 2020)

adm <- rbind(adm_17, adm_18, adm_19, adm_20)
...
inst_adm <- inst_adm %>%
  ...
  select(year, unitid, instnm, n_applicants, n_admits, n_enrl, test_req, n_sat, 
...
Get the IPEDS Enrollment data for institutions of interest for survey years 2017-2020 and process to create a tidy dataset.
...
enrlmnt_19 <- read_csv(here('data', 'ipeds_2019', 'ef2019a.csv')) %>% 
  janitor::clean_names() %>% mutate(year = 2019)
enrlmnt_20 <- read_csv(here('data', 'ipeds_2020', 'ef2020a.csv')) %>% 
  janitor::clean_names() %>% mutate(year = 2020)

enrlmnt <- rbind(enrlmnt_17, enrlmnt_18, enrlmnt_19, enrlmnt_20)
...
inst_enrlmnt <- inst_enrlmnt %>%
  ...
  rename(n_enrl = eftotlt,
         n_enrl_m = eftotlm,
         n_enrl_w = eftotlw) %>%
  select(year, unitid, instnm, lstudy, n_enrl, n_enrl_m, n_enrl_w) %>%
...
Get the IPEDS Completions data for institutions of interest for survey years 2017-2020 and process to create a tidy dataset.
...
compl_19 <- read_csv(here('data', 'ipeds_2019', 'c2019_c.csv')) %>% 
  janitor::clean_names() %>% mutate(year = 2019)
compl_20 <- read_csv(here('data', 'ipeds_2020', 'c2020_c.csv')) %>% 
  janitor::clean_names() %>% mutate(year = 2020)

compl <- rbind(compl_17, compl_18, compl_19, compl_20)
...
inst_compl <- inst_compl %>%
  ...
  rename(n_deg = cstotlt,
         n_deg_m = cstotlm,
         n_deg_w = cstotlw) %>%
  select(year, unitid, instnm, awlevelc, n_deg, n_deg_m, n_deg_w) %>%
...
list_of_datasets <- list("Institution Characteristics" = inst_list,
                         "Admissions" = inst_adm,
                         "Enrollment" = inst_enrlmnt,
                         "Completions" = inst_compl)

write.xlsx(list_of_datasets, file = here("data", "IPEDS_dataset.xlsx"), overwrite = TRUE)
