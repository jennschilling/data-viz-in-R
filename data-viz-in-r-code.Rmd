...
inst_adm <- inst_adm %>%
  ...
  select(unitid, n_applicants, n_admits, n_enrl, test_req, n_sat, pct_sat, sat_score, n_act, pct_act, act_score)
...
inst_enrlmnt <- inst_enrlmnt %>%
  ...
  rename(n_enrl = eftotlt,
         n_enrl_m = eftotlm,
         n_enrl_w = eftotlw) %>%
  select(unitid, lstudy, n_enrl, n_enrl_m, n_enrl_w) %>%
...
compl <- read_csv(here('data', 'ipeds_2019', 'c2019_c.csv')) %>% janitor::clean_names()
...
inst_compl <- inst_compl %>%
  ...
  rename(n_deg = cstotlt,
         n_deg_m = cstotlm,
         n_deg_w = cstotlw) %>%
  select(unitid, awlevelc, n_deg, n_deg_m, n_deg_w) %>%
